...
`research/` is measured on; two are retired on the cold path and survive only in the three
warm-entry recipes, which by construction have no bootloader; one is an ablation that is off by
default and in no recipe.
