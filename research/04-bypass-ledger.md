...
**Still faking something, on some path.** **Two** are live on the retail path every number in
`research/` is measured on — #6, the synthesised co-processor replies, and **#18, the boot
ROM's handoff remap**, added 2026-09-06. #18 costs nothing measurable — every oracle in this
repository is identical with it and without it — which is exactly why it needs a row: a bypass
that moves no number is the kind that stops being noticed.

**#19 is live on NO recipe at all, and that is the point of its row.** Added 2026-09-07, it fakes
the part's *speed* and it lives only inside `ipod-gui`, because a window is a thing a person watches
in real time and a recipe is not. Every `trace` / `ipod-boot` / `ipod-film` invocation pins its own
`--clock=`, so every figure in this directory is taken on the machine it was always taken on. A
bypass whose blast radius is one front end still gets a row: this file's rule is that no shortcut
becomes permanent by being unwritten, not that a shortcut only counts when it moves a number here. #4 and #11 are retired on the
...
| 18 | **the boot ROM's handoff remap** — MMAP window 0 as `logical 0 -> physical 0x10000000`, installed by the machine when control first reaches SDRAM on a cold boot | **the three cold recipes** — `cold-boot.sh`, `retail-boot.sh`, `flash-update.sh`, i.e. anything passing `--cold-boot`. Not a flag by default: `map_hardware` arms it and `Machine::run` consumes it. `--no-boot-handoff` is arm B and reproduces the defect on demand | the state Apple's bootloader leaves when it enters an image: logical `0` stops being the NOR and becomes the image the ROM just loaded | **every image the ROM can enter needs its own exception vectors at logical 0, and nothing in this dump installs the window that puts them there.** `diag`'s click wheel is interrupt-driven and the ISR has exactly one caller chain, rooted at the image's own IRQ vector slot: `0x18 -> 0x100002a4 -> 0x10000880 -> 0x10009e40 -> 0x10008424`, which is the only code in the image that reads `CLICKWHEEL_DATA`, acks it, and writes the button byte the 150 ms poll consumes. `diag` enables IRQ 40 itself (`CPU_HI_INT_EN = 1 << 8` from `0x100088d4`) and runs with `I` clear. `disk` is built the same way. Leave the NOR at `0` and the first frame vectors into Apple's forwarder (`mov pc, #0x40000018`) and then into the `b .` the ROM ships in IRAM: measured, SELECT+REW released, `irqs: 3 073 043 asserted, 1 taken` with `r15=0x40000018` and 76 800 of 76 800 pixels lit — a redraw stopped half way, not a screen | 🟡 | **the instruction, or the hardware.** Nothing here programs the unit: `--watch-range=0xf000f000:0x40` counts **0** writes across a 400 M chord boot against **56** from RetailOS's own crt0 at `0x10000208`, and a word scan of the ROM's whole IRAM image (NOR `0x8000..0x20000`) for `0x3a00`/`0x3c00`/`0x3e00`/`0x0f84`/`0x3f84` finds none — its four `0xf000f000` literals all address `+0x40`/`+0x44`, the cache mask and operation. The handoff itself is three stores and a branch (`0x40000050` = entry, `COP_CTL` = 0, `bx r0`). **And it cannot be done at reset**: Apple's bootloader reads its own SysCfg at logical `0x405c` while running out of IRAM, so a machine with SDRAM at 0 from instruction 0 prints `BootLoader running on ??? cpu PP5022-C` and never loads an image — which is also what Rockbox's `hwcompat.h` says, reading `0x0000405c` in a `BOOTLOADER` build and `0x2000405c` after its own remap. Two independent firmwares say the ROM leaves a window anyway: `ipodloader2`'s `remap_memory()` **saves the eight words at `0xf000f000` and restores them** before handing on, and programs windows **1 and 2** rather than 0. Encoding is Rockbox's own (`LOGICAL 0x00003c00` / `PHYSICAL 0x10000f84`). Retire it by finding the instruction — or by measuring what `0x00000018` holds on real hardware while `diag` runs. **Cost of installing it: nothing measurable.** Pinned `--battery=100 --rtc=2026-09-06T12:00:00`, both arms: retail cold boot **899 999 952 instructions / 341 349 asserted / 166 817 taken / 766 ATA**, cold Rockbox **44 514 927 / 1 227 / 265 / 2 070 ATA / 10 frame updates**, `flsh` diag **2 / 1 / 70 669 px**, the chord held **0 / 0 / 70 669 px** — all four identical before and after. The two arms that move are the ones that were broken. See [research/07](07-the-flash-images.md) §"The storm was the vector table" |

| 19 | **the window underclocks the iPod** — `ipod-gui` runs the machine at the clock this computer was measured to sustain, not at the PP5021C's 75 | **`ipod-gui` only, from the end of its first cold boot onward.** A launch starts at `settings.txt`'s `sustained_clock`, or at [`CLOCK`] where there is none; the cold boot measures this host, and at the boot's end the machine moves to what it measured and the number is written down for next time. Before 2026-09-07 only the writing-down happened, so the run that paid for the measurement never saw it and a fresh profile ran its whole first session at 75 — the slowest this program ever is, on the one run a person judges it by. `trace`, `ipod-boot`, `ipod-film` and `tools/clean-run-matrix.sh` pin their own `--clock=` and never read any of it; a window launched with `--clock=` is unaffected too, and `--headless=` pins for the same reason | the part's *speed*. A 5G is 75 MHz; a window that has measured this host at ~16 M interpreter steps a second says the part is 16 MHz and runs its seconds in real seconds | **a correctly clocked iPod in slow motion is less faithful than an underclocked one in real time.** At clock 75 this host runs the machine at 0.07–0.15x of life, and `emu::drain` spaces wheel steps by 4 ms of the *machine's* time — so a sixty-detent scroll a thumb made in 1.00 s took **4.76 s** to reach the wheel and seven of the sixty never arrived. At clock 16: **1.11 s**, all sixty, and a resting machine at **1.00x**. Slow motion is a thing no iPod has ever done; dropping frames on a slower part is a thing every iPod has done. **The operator's `6 detents felt` is NOT this** — that is RetailOS's own scroll accumulator folding ~11 wheel positions into one event, it is identical at both clocks, and research/10 Addendum 33 §4 retracts the issue's reading of it | 🟡 | **issue #33 — close the interpreter's gap so a host retires 75 M steps a second.** `pace::sustainable` then answers `CLOCK` on its own, the window runs the part's own rate, and this row retires without a line of code being deleted. The condition is mechanical and checkable today: delete `sustained_clock` from `settings.txt` on a fast enough host and the next boot re-measures 75. **Cost of installing it: nothing on any recipe.** Pinned `--battery=100 --rtc=2026-09-06T12:00:00`, FLASH/DISK/BUDGET explicit in both arms: retail cold boot 900 M @`--clock=5` **899 999 952 / 341 349 / 166 817 / 766 ATA**, cold Rockbox 200 M **44 514 927 / 1 227 / 265 / 2 070 ATA / 10 updates**, `flsh` diag **8 frame updates / 70 669 px**, the chord held 400 M **12 commands / 10 updates / 70 669 px / 3 of 3 steps** — all four identical, and their framebuffer dumps compare byte-for-byte equal. See [research/10](10-the-resource-image.md) Addendum 33 |
...
| 7 | `COP_STATUS` sticky `0x80000000` — **retired 2026-08-19, REOPENED 2026-08-26, RE-RETIRED 2026-09-05** | **off the default path again.** `trace.rs` installs the override whenever the coprocessor is off, and two cores is the default once more — so no recipe carries it unless it asks for `--no-second-core` | the coprocessor is permanently asleep | we ran one core; firmware wakes the COP and waits for it to sleep again | 🟢 | **Done — a second ARM7TDMI runs from the reset vector, and the default was flipped on evidence rather than on principle.** Both cores enter at the same address; the coprocessor reads `PROC_ID`, decides it is `0xAA`, and takes Apple's seven-instruction path at `0x8054` — park at `COP_CTL`, two nops, `ldr pc, [0x40000050]`. **The one thing that had to be got right was concurrency, not registers**: Apple's bootloader writes that entry vector *two instructions* before the wake, and with a 1000-instruction quantum the CPU ran on into the OS and overwrote it, so the coprocessor read an instruction word, jumped to it as an address and wandered 27 660 256 code buckets. A wake now ends the running core's turn. **At the moment of flipping, every recipe measured here is identical with one core and two** — retail **599 ATA commands / 2 916 non-black pixels**, cold-booted Rockbox **10 304 / 74 057**, in both arms — so the flip re-baselines nothing and every number already in `research/` stands. `--cop-trace` gives the coprocessor the instruments the CPU has had all along, and `--no-second-core` remains as arm B: not a bypass, but a smaller machine than the part, which is an honest thing to be able to ask for. **⚠️ THE PREMISE EXPIRED — default flipped back 2026-08-26.** *"Every recipe is identical with one core and two"* was true when written and is not true now. Same NOR dump, same `PRISTINE` drive, `BUDGET=900000000`: `ipod-boot retail` **766 ATA** on one core and **70** on two; `ipod-emulator --headless` **769 ATA / 75 267 lit / the language picker** on one core and **70 / 2 916 / Apple's logo** on two. The same factor of eleven in **both front ends**, so it is the coprocessor and not a harness. The one-core boot moved a long way since the flip — 599 ATA and 2 916 pixels then, 769 and 75 267 now — and the two-core boot did not come with it. A default that stalls RetailOS at the logo silently re-baselines everything measured through `trace`, which is what `KNOWN-BUGS.md`'s *"`ipod-boot retail` and `ipod-gui` do not boot the same machine"* had been recording since 2026-08-25 without knowing the cause. `--second-core` is now opt-in and is where the defect lives; one core is the default again, which puts this row's override back on the retail path — so **row 7 is live again, not retired**, and the honest state is 🟡 rather than 🟢. **✅ RE-RETIRED 2026-09-05 — the cause was found and it was not the coprocessor.** A word read of `CPU_QUEUE` took `read32`'s fast path, which returns the stored bytes and never reaches the clear in `read8_inner`, so the mailbox interrupt was taken, the queue read, and the interrupt taken again for the whole budget — **2 953 894 reads of `0x60001010` and 2 953 894 interrupts taken, exactly equal**, which is what a read that fails to clear its source looks like from outside. Naming the mailbox page in `page_is_plain` fixed it; `research/03` §57 is the account. The 2026-08-26 numbers now read the other way and then some: retail cold boot **619 one core / 620 two**, a wheel descent **705 / 769**, and where one core reads **5 of 31** posted wheel frames and drops 25 and then stores through a null pointer onto the IRQ vector at `0x18`, two cores read **31 of 31** and never do. Cold-booted Rockbox is **identical** in both arms — 3977 ATA, 74 057 lit — so the oracle is unmoved and every number measured on one core still stands. Two cores is the default, this override is off the measured path, and the row is retired on cause rather than on a premise that might expire again. `--no-second-core` is still arm B and now genuinely is one: it was named here and in `ipod-boot`'s `MACHINE_SHAPING` and **nothing ever parsed it** until the same change, so every run that asked for one core by that spelling had silently been getting whatever the default was. **⚠️ AND ARM B DID NOT EXIST UNTIL 2026-09-05 EITHER.** `map_hardware` installs this override gated on `second_core`, and in `trace.rs` it ran at line 623 while the flag that set `second_core` was parsed at line 1164 — so **the gate read the default and nothing else**. While one core was the default that means a `--second-core` run got the override *as well*: the arm that this row's own justification calls arm B was carrying the bypass it was supposed to ablate, on the path that produces every number in `research/`. The core count is now decided beside `--cop-awake`, before `map_hardware`, and the per-run banner finally disagrees between the arms — `bypasses live: 3` with two cores and `4` with `--no-second-core`, #7 named only in the second. Found because flipping the default turned the same defect the other way round and Rockbox stopped booting entirely: **0 ATA commands against 3977**, nothing drawn, the wheel's receiver never armed. Both arms now boot Rockbox to 3977 ATA and 12 distinct pictures under a six-detent wheel script |
...
- **The memory model** — SDRAM sizing, aliasing and the remapped views are modelled, not faked.
  *(One window is: see #18. Every remap a firmware **writes** is honoured; the one Apple's
  bootloader leaves without writing is supplied.)*
...
0. **#11 — RETIRED.** The MMAP unit is modelled from `ipodloader2`'s encoding; logical 0 maps to
   SDRAM when the firmware programs it, and RetailOS executes. *(**Half of that sentence turned out
   to be the assumption behind #18** — 2026-09-06. "When the firmware programs it" is true of
   RetailOS, Rockbox and `ipodloader2`, and false of Apple's own `diag` and `disk`, which never
   touch the unit and still need their exception vectors at logical 0. Nothing in the retail ROM
   programs it either. #18 supplies the window the handoff must be leaving; the retirement of #11
   stands, because the unit is real and firmware-driven — what was missing is a window nobody
   writes.)* Originally: Retired 2026-08-13; it was the checksum bug. The image now loads
