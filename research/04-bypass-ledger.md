...
| 7 | `COP_STATUS` sticky `0x80000000` | **ALL SIX recipes — and it is not a flag.** `trace.rs:2102` pushes it into `read_overrides` with no way to ask for it, *but only `if read_overrides.is_empty()`* — so **any** `--rdval` silently suppressed it. That is why it was OFF in `flsh.sh` and `flash-update.sh` until 2026-08-14, and is on in all six now that no recipe passes `--rdval` | the coprocessor is permanently asleep | we run one core; firmware wakes the COP and waits for it to sleep again | 🟡 **cost measured 2026-08-13** | emulating the second core. **RetailOS reads `0x60007004` 5 470 times and runs Rockbox's `wake_cop` verbatim at `0x000cfb20` — read `COP_CTL`, clear bit 31, write back — 10 198 times.** The override is unconditional, so the write-back is discarded and the retry can never succeed. It is not, however, a mailbox conversation: `--readlog` + `--storeaddr` over all twelve words of `0x60001000..0x6000102f` record **zero reads and zero writes**, with `0x60007004`/`0x60007000` as positive controls in the same logs. **Re-measured 2026-08-14 over the full 600 M window and it survives — but neither that control nor the 2026-08-13 re-confirmation's `0x60005010` established it, and the reason is worth carrying: `read_log` caps at 2 000 000 entries, `0x60005010` is read 9 588 012 times, so the log saturated shortly after @123 M and reported a clean zero for a run it had stopped watching. `0x60005010` was also served by `read8` (a `page_is_plain` exclusion) where `0x60001000` is served by `read32` — wrong code path as well as too loud. With low-rate, word-width, plain-page controls (`0x60004020`, `0x6000603c`, `0x70000000` reading; `0x60004024` storing) the log totals **74 reads and 13 stores**, the controls fire from @88 375 to @599 999 951, and **not one entry is in `0x600010xx`**. The mailbox really is silent. See [research/10](10-the-resource-image.md) Addendum 14 §8.** RetailOS does arm the line — `0x001fc9a4` writes bit 4 to `CPU_INT_EN` — and is simply never sent anything. See [research/10](10-the-resource-image.md) Addendum 7 §6. **2026-08-17 — the row is now measurable, and the protocol has been found. It is shared memory, which is why the mailbox looked silent.** `--cop-awake` stops installing the override, so the arm where the model does not lie about the second core exists for the first time. **On the DRM path it changes nothing**: both arms are identical to the instruction — 25 506 code buckets, the same call at 55 050 392 and the same failing return at 1 628 342 943 — and `wake_cop` does not execute until **1 700 032 431**, *after* the keys have already been refused. Whatever the coprocessor is for, it is not what refuses them. What it *is* for is now visible. `--regs-at=0x000cfb20:3` shows the caller is always `0x000c17fc` (`r14 = 0x000c1800`), and `--watch-writes` over the object it carries in `r5`/`r10` names the eight stores that precede the call — `0x000c166c`, `0x000c16a0`, `0x000c16dc`, `0x000c1714`, `0x000c174c` (344 byte-writes, the payload), `0x000c1774`, `0x000c179c`, `0x000c17d0`. **A function builds a 0xD8-byte descriptor in SDRAM and then wakes the second core**, which is exactly why `0x60001000..0x6000102f` records zero traffic: the request is not in the mailbox registers, it is in RAM, and the mailbox measurement was answering a question about the wrong medium. The descriptor sits at **`0x149370c0`** — 0xD0 below the DRM context's flag byte at `0x14937190`, inside the same allocation. It is asked for **16 667 times** between 1 700 032 431 and 2 553 331 396 and can never be answered, because the override discards the wake write. **Retiring #7 therefore means running a second ARM7TDMI from reset** — `PROC_ID` answering per-core rather than globally, a per-core interrupt mask, and an interleave — after which the descriptor above is the thing the second core will be found consuming |
