...
| 15 | ~~MMAP window size fixed at **64 MB**~~ **RETIRED 2026-08-13** | **no recipe** — it was a hardcoded constant in `lib.rs`, now decoded | the size/mask field of every MMAP window | the base addresses decoded from `ipodloader2`; the size field did not, so it was hardcoded | 🟢 | **Decoded, not guessed.** Rockbox's `crt0-pp.S` names the halves: the size is a `MASK` in **LOGICAL** bits 13:4 (bit *m* compares address bit *m+16*), and PHYSICAL holds only R/W/DATA/CODE `FLAGS`. The `0x0f84`-vs-`0x3f84` difference this row blamed is PP502x vs PP5002 — a part number, not a size. RetailOS's SDRAM window leaves address bit 26 uncompared, so it answers for `0x04000000` as well as `0x00000000`; that don't-care is exactly what it was reading through. See [research/11](11-rtxc-and-the-video-coprocessor.md) §33 |
| 13 | ~~`ipodloader2`'s `loader.bin`~~ **NOT IN USE** | **no recipe, and never was executed** — it is a documentation source | Apple's bootloader's handoff | it has **zero `aupd` handling**, so it was kept as a way to bisect the blocker | ⚪ | **Nothing — the retirement condition is already met.** `cold-boot.sh` boots Apple's own retail bootloader out of the NOR dump, entering at `0x0`; the console receipt is `BootLoader running on iPod M25 … Retail mode … Running 'osos' 0 from 0x10000000`. `ipodloader2` is a *documentation source* (its MMAP encoding and register names), never executed. See [research/09](09-retailos-boot.md) §Provenance |
| 16 | **— NEVER EXISTED.** The number was skipped, not vacated | **n/a** | nothing — there was never a bypass here to fake anything | **The gap is a miscount, and it is checkable.** `git log -p --all -- research/12-bypass-ledger.md` contains **no row 16 in any revision**, and `git log -p --all` across the whole repository contains **no `#16` at all** except ARM shift amounts (`mov r4, r4, lsr #16`). At `f7348b6^` the table read `1…12, 14, 15, 13` — fifteen rows whose highest number was 15 — and `f7348b6` added the VideoCore ablation as **17**. Every other retirement in this file left a struck-through tombstone row; this one has no tombstone because there is nothing to entomb | 🟢 n/a | n/a — this row exists so the gap stops reading as a silent deletion, which is the failure mode the whole file is against |
| 17 | `--force-vc-upload` · `--force-vc-retire` — **OFF BY DEFAULT, NOT IN ANY RECIPE** | **no recipe, verified 2026-08-14** — `grep force-vc tools/ipod-boot/*.sh` is empty, and `trace.rs` prints `BYPASS #17 active` when either is given, which no recipe's output carries | the VideoCore transfer engine's two completion signals: RTXC semaphore `0xe0`, and the four in-use bytes at `channel+0x18` | **an ablation, deliberately kept known-wrong.** `APPLEBOOT` blocks forever uploading `vmcs.bin`; this opens the door to see what is behind it. It answered its question — see below — and its only remaining use is re-running that experiment | 🔴 **by construction** | **the transfer engine at `0x60009000`, and enough of the VideoCore to answer a request.** Retiring it is not optional-by-taste: with both halves on, RetailOS gets two calls further and then waits on `0xea` for a *reply* from the co-processor (research/20 Addendum 8 §6–7). **That day is 2026-08-13** — `0x60009000` is modelled, moves bytes, and its descriptors retire without help (research/20 Addendum 9 §6: the in-use ring drains to `[0,0,0,0]` with the emulator never touching it, because the real driver's ISR owns it). The model reproduces this ablation's every measured number — 62 tasks, 256 ATA commands, 302 DMA transfers — and additionally delivers the firmware, which the ablation never did. **Both flags are now redundant and should be deleted;** they are left in only until someone wants to re-run the comparison. **Never add either to `retail-boot.sh`, `cold-boot.sh` or `warm-boot.sh`** — every number in research/ is measured without them, and a recipe that carries an ablation stops being a measurement of the machine |
...
Four artefacts, all derived field-by-field from RetailOS's own reader ([research/20](20-the-resource-image.md)
...
4. **Surfaces are allocated from `0xE0000` upward — on Rockbox's authority, not the
   ...
   **Attacked 2026-08-14 and it is now a *known-wrong* choice, still in place.** `0xE0000` is the
   ...
   ([research/24](24-the-apple-logo.md)). A real co-processor would not hand that address out as a
...
1. **Establish where a surface really lives.** Either from the co-processor's own code (the export
   ...
   [research/21](21-the-videocore-runtime.md)), or by an ablation: move the allocator's base and see
...
[research/22](22-how-retailos-draws.md).**
...
| Knob | Effect | Note |
|---|---|---|
| `--clock=N` | instructions per simulated microsecond; 75 is real time | lowering it makes time outrun the code, collapsing timeout waits. **Timing-sensitive code can notice** |
...
- **ATA bus-master DMA** — the engine at `IDE_BASE+0x400/0x408/0x40c`, modelled from Apple's own
  bootloader programming it (see [research/11](11-rtxc-and-the-video-coprocessor.md) §26). `0xc8`
...
0. **#11 — RETIRED.** The MMAP unit is modelled from `ipodloader2`'s encoding; logical 0 maps to
   ...
   gap is **one 512-byte sector**. See [research/11](11-rtxc-and-the-video-coprocessor.md) §27.
0a. **#15 — RETIRED.** The window encoding is Rockbox's documented one, not a hardcoded size. With
   ...
   BCM. See [research/11](11-rtxc-and-the-video-coprocessor.md) §33.
...
1. **THE BOOT LOOP IS A MISSING FONT, AND THE FONT IS IN `rsrc`.** RetailOS asks its font registry
   ...
   is now *why it stops after the MBR*. See [research/20](20-the-resource-image.md).
...
1z. *(superseded)* **THE BOOT LOOP DOES NOT EXIST.** It was never a bypass artefact and never an unconstructed
   ...
   **one** arrival at `0x00000000` — the cold reset — under `--pmu` *and* under the old
   ...
   See [research/11](11-rtxc-and-the-video-coprocessor.md) §40. **#6 remains the right next device**
   ...
   [research/11](11-rtxc-and-the-video-coprocessor.md) §35). Something constructed nothing. These
...
0c. **Where the boot actually is — re-measured 2026-08-14.** One command:
   ...
   >    ever created. See [research/11](11-rtxc-and-the-video-coprocessor.md) §45.
   ...
   >    §35's crash (that was the sector shift). See [research/11](11-rtxc-and-the-video-coprocessor.md) §44.
...
1a. **The sharpest open defect is the block on RTXC semaphore `0xd1`.** `MP3ExampleTask`
   ...
   2 G → 6 G moves nothing. See [research/20](20-the-resource-image.md) Addendum 11 §5.
...
0d. **#12 — RETIRED on the retail ROM.** The bypass was hand-applying the updater's own last write.
   ...
   [research/17](17-the-flash-images.md). **Re-run 2026-08-14 with #1, #2 and #3 removed from the
...
2. **#13** — retires when the ROM's halt is understood. **#12 remains open on the prototype ROM.**
3. — *(#6 promoted to 1; executing `vmcs` is the difference between "a frame appeared" and "the
   display works")*
4. **#9** — the disk path, once something depends on sustained throughput. *(**#10 is retired**, so
   this line no longer names a pair. #9 is now live-and-unswitchable in every recipe rather than
   flag-selected, which raises its priority rather than lowering it: nothing in a recipe reveals it.)
...
6. **#7** — the second core; only matters when something needs the COP.
...
**Our own [research/15](15-the-chip-inventory.md) §NOR already said `WF` wins** — iPodLinux and the
...
## #17 — what the VideoCore ablation bought, and why it must stay off
...
acknowledged. It needs the co-processor to answer. See [research/20](20-the-resource-image.md)
...
[research/19](19-what-the-hardware-must-supply.md): a heap record's delegate field reported as
