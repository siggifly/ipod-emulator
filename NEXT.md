...
|---|---|---|
| `--until=NNNs` / `NNNms` | ends a run when the **simulated clock** reaches it, whatever the instruction budget | **The instrument the other rows in this table needed and did not have.** `BUDGET` counts OUR work; every deadline the firmware waits on is stated in ITS time, and `7e30c1f` (*the clock stops inventing time*) changed the exchange rate between them by about five. Every budget written before it now buys a fifth of the run its author meant — which is how the Brick descent that reached a dealt game of Solitaire on 2026-08-18 stops on the language picker today, having fired all 106 of its steps and reported nothing wrong. `--wheel`'s anchors learned this on 2026-09-01 and were re-based on simulated time; the budgets are the same mistake in the same units and were not. **State a run in the firmware's clock**: `--until=200s` is 200 seconds of iPod at any `--clock`, on any machine, whatever changes underneath it, and `BUDGET` becomes the ceiling that stops a wedged run rather than the thing that decides what a run covers. The ceiling is the clock's own: `usec` is a `u32`, so `--until` above ~4 295 s is refused rather than wrapped |
...
| `--no-boot-handoff` | ablates ledger #18, so Apple's bootloader hands off with logical `0` still answering NOR | **arm B for the one ledger row whose evidence is a behaviour rather than an instruction.** With it, `ipod-boot retail --clickwheel` with the SELECT+REW chord *released* reproduces the storm exactly: `irqs: 3 073 043 asserted, 1 taken`, `r15=0x40000018`, 5 commands / 3 frame updates, and 76 800 of 76 800 pixels lit. Without it, `2 / 1`, 12 / 10, and 70 669. The banner counts `bypasses live: 4` and `3` in the two arms, so a run says which machine it was |
| `--rtc=YYYY-MM-DDTHH:MM:SS` | pins the PMU's clock the way `--battery=N` pins the charge | **the host clock is an input to the machine, and without this no instruction count is a baseline.** The PMU's RTC is seeded from `date`, the firmware reads it, and two runs of one recipe hours apart are two machines: cold-booted Rockbox on the SAME binary with `--battery=100` gave **44 511 132** instructions at one time of day and **44 509 887** at another. ATA commands, IRQs taken and frame counts did not move — but "these numbers happen to be stable" is not the same as a pinned comparison, and a before/after that rests on it is one nobody else can rerun. `54d5dce` had already recorded two films of the same command disagreeing on frames 2-11 for this reason. Anything that does not parse falls back to the host clock rather than to a made-up date; the weekday byte is computed, and `the_pinned_clock_parses_and_dates_itself_correctly` checks it against `date +%u` on a Sunday, a January date, the March boundary and a leap day |
