...
| Rockbox 4.0 | **boots to its main menu and takes wheel input** (2026-08-18), and **cold-boots to it** through Apple's bootloader and its own (2026-08-19) — same 10 304 ATA commands and 74 057 lit pixels with one core or two. Doom **launches and initialises but does not play** (2026-09-06). Rockbox reaches it by shortcut, its three assets are now installed by the harness instead of demanded of a hand-prepared drive, and it scrolls its whole startup — `R_InitPlanes`, `I_InitSound: sound module ready`, `Starting Graphics engine`. Then the panel stops: **last drawn at 153 s of a 2 400 s run**, all 30 wheel steps fired. It briefly read as PASS because the matrix counted pictures and that console is 29 of them; the row asks whether it is still drawing at the end now. `i_video.c:1175` sets `noprintf=1` immediately after that last message, so anything that fails after it fails silently. No sound |
