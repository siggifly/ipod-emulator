...
/// cannot lose its reason on the way across, which `primitives.slint:486` states as the
...
mod tests {
    ...
    ///
    /// `primitives.slint:42` declares a non-empty `reason` as the invariant on a disabled
