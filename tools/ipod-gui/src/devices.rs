...
// are the bench's cradle as well.** `window.slint:970` reads `root.current.cradle-label` and
// `window.slint:1008` reads `root.current.startable`, so the sentence that refuses this page's
...
///    refusal slot: `primitives.slint:726` is `text: root.enabled ? root.consequence : root.reason`,
