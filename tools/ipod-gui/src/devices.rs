...
// are the bench's cradle as well.** `window.slint:832` reads `root.current.cradle-label` and
// `window.slint:865` reads `root.current.startable`, so the sentence that refuses this page's
...
///    refusal slot: `primitives.slint:639` is `text: root.enabled ? root.consequence : root.reason`,
