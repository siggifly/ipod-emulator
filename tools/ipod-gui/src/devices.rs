...
// are the bench's cradle as well.** `window.slint:873` reads `root.current.cradle-label` and
// `window.slint:890` reads `root.current.startable`, so the sentence that refuses this page's
